...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const anglesDiff = Math.abs(angle2 - angle1);

  return anglesDiff > Math.PI ? 2 * Math.PI - anglesDiff : anglesDiff;
}
...
function roundToPowerOfTen(num, pow) {
  const digitPow = 10 ** pow;
  return pow ? Math.round(num / digitPow) * digitPow : num;
}
...
function getFibonacciNumber(index) {
  let fibonacciNumberPrev = 0;
  let fibonacciNumberNext = 1;
  let fibonacciNumber;
  if (index === 0) return fibonacciNumberPrev;
  if (index === 1) return fibonacciNumberNext;
  for (let i = 2; i <= index; i += 1) {
    fibonacciNumber = fibonacciNumberPrev + fibonacciNumberNext;
    fibonacciNumberPrev = fibonacciNumberNext;
    fibonacciNumberNext = fibonacciNumber;
  }
